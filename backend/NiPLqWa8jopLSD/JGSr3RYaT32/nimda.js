...
router.post('/sitesettingupdate',common.admintokenMiddleware,(req,res)=>{
  try{
    if(req.body.page=="site"){
      ...
      sitesettings.findOneAndUpdate({},{"$set":{"address":req.body.address,"mail":req.body.mail,"phone":req.body.phone,"facebook":req.body.facebook,"twitter":req.body.twitter,"instagram":req.body.instagram}},{new:true}).exec(function(errss,resdata){
        if(errss) {
             console.log(errss);
...
router.post('/adminloginhistory',common.admintokenMiddleware, common.originMiddleware, (req, res) => {
  try{
    ...
    var search  = req.body.search;
    if(req.body.search != ""){
      cnt = {$or:[{'username': { $regex: '.*' + search + '.*',$options: 'i' }},{'emailid': { $regex: '.*' + search + '.*',$options: 'i' }},{'ip_address': { $regex: '.*' + search + '.*',$options: 'i' }},{'browser': { $regex: '.*' + search + '.*',$options: 'i' }}]};
    } else {
      cnt = {};
    }
     async.parallel({
...
router.post('/IPblockhistory',common.admintokenMiddleware , common.originMiddleware, (req, res) => {
  ...
  var search = req.body.search;
  if(req.body.search != ""){
    cnt = {$or:[{'emailid': { $regex: '.*' + search + '.*',$options: 'i' }},{'user_ip': { $regex: '.*' + search + '.*',$options: 'i' }}]};
     filter = parseFloat(search);
         if(filter >= 0){
...
// router.post('/profitexport',common.admintokenMiddleware, common.originMiddleware, (req, res) => {
//   try{

//     var cnt;
//     var search = req.body.search;
//     if(req.body.search != ""){
//       cnt = {$or:[{'username': { $regex: '.*' + search + '.*',$options: 'i' }},{'currencyname': { $regex: '.*' + search + '.*',$options: 'i' }},{'pairs': { $regex: '.*' + search + '.*',$options: 'i' }},{'profit_type': { $regex: '.*' + search + '.*',$options: 'i' }}]};
   
//         filter = parseFloat(search);
//          if(filter >= 0){
//             cnt['$or'].push({'profitamt': { $eq: parseFloat(filter)}},{'total_amount':{$eq:parseFloat(filter)}},{'price':{$eq:parseFloat(filter)}},{'fees':{$eq: parseFloat(filter)}});
//                  } 
//     }
...
// router.get('/getallnotification', common.originMiddleware ,common.tokenMiddleware,(req,res)=>{
// pushnotify.find({"user_id":req.userId,"status":"Active"}).exec(function(err,resData){
// res.json({status:true,data:resData})
// });
// });

// router.get('/getallnotificationapp', common.originMiddleware,common.tokenMiddleware,(req,res)=>{
// pushnotify.find({"user_id":req.userId}).exec(function(err,resData){
// let val=resData
// var arr=[]
// var countsdoc=0
// if(resData)  {
...
// }
// countsdoc=arr.length
// res.json({status:true,data:resData,count:countsdoc})

// }
// else{
//   res.json({status:false,Message:"No Notification found"})

// }
// });
// });

// router.post('/updatestatus_notify', common.originMiddleware ,common.tokenMiddleware,(req,res)=>{
//   try{
//     pushnotify.findOneAndUpdate({"user_id":req.userId,"_id":req.body._id},{"$set":{"status":"Deactive",}},{new:true}).exec(function(err,resData){
//       res.json({status:true,Message:"Notification status updated"})
//     });
//   }
//   catch(e){
//     console.log("updatestatus_notify",e)
//   }
// });
  ...
  try {
    ...
    pairs.find({"pair":pair }).exec(function(err,resData){
      if(resData.length > 0) {
        ...
        currency.findOne({currencySymbol:values.fromCurrency},(err_f,f_cur)=>{
          if(f_cur){
            currency.findOne({currencySymbol:values.toCurrency},(err_s,s_cur)=>{
              if(s_cur){
                var obj     = {
                  "pair"                 : values.fromCurrency+"_"+values.toCurrency,
                  "fromCurrency"         : values.fromCurrency,
                  "toCurrency"           : values.toCurrency,
                  "fromCurrency_i"       : f_cur.currency_image,
                  "toCurrency_i"         : s_cur.currency_image,
                  "fromCurrency_address" : f_cur.contract_address,
                  "toCurrency_address"   : s_cur.contract_address,
                  "fromCurrency_type"    : f_cur.currency_type,
                  "toCurrency_type"      : s_cur.currency_type,
                  "fromCurrency_name"    : f_cur.currencyName,
                  "toCurrency_name"      : s_cur.currencyName,
                  "fromCurrency_decimal" : f_cur.currency_decimal,
                  "toCurrency_decimal"   : s_cur.currency_decimal,
                  "created_by"           : "admin",
                  "created_by_address"   : "admin_address",
                }
                ...
                res.json({status:true,msg:'Not found '});
              }
            })
          }else{
            res.json({status:true,msg:'Not found '});
    ...
    router.post('/insert_cherry', common.originMiddleware , async function(req,res){
      try {
        let values  = req.body;
            ...
            // return;
            cherry_list.create(obj,async function (err,resdata){
              if (err) return res.status(500).send(err);
              if(values.type == "pair"){
...
// Banner section
 router.post('/bannerlist', common.admintokenMiddleware , common.originMiddleware , async function(req,res){
      try {
        var skip    = req.body.page.pageNumber *  req.body.page.size;
        var limit   = req.body.page.size;
        let prop1   = req.body.sorting.prop;
        ...
        var search  = req.body.search;
        if(req.body.search != "") {
          cnt = {$or:[{'heading1': { $regex: '.*' + search + '.*',$options: 'i' }},
                      {'heading2': { $regex: '.*' + search + '.*',$options: 'i' }},
                      {'heading3': { $regex: '.*' + search + '.*',$options: 'i' }}]};
        }

        async.parallel({
          bannerTotalCount:function(cb) {
            banner.find(cnt).countDocuments().exec(cb);
          },
          bannerData:function(cb) {
            banner.find( cnt ).limit(limit).skip(skip).sort( srt ).exec(cb)
          },
        },function(err,results){
          if (err) return res.status(500).send(err);
          apiresponse.data        = [];
          ...
          apiresponse.bannercount    = results.bannerTotalCount;
          apiresponse.message     = "";
          res.json(apiresponse);
        })
      } catch(e) {
     ...
     // update banner
    router.post('/editbanner',common.admintokenMiddleware, common.originMiddleware, (req, res) => {
      banner.findOneAndUpdate({ "_id": req.body._id },
        { "$set": 
        { "image"      : req.body.image,
          "button_txt"        : req.body.button_txt,
        ...
        exec(function(err, resUpdate){
          try {
            if(err) {
              ...
              banner.find({ "_id": req.body._id }).exec(function(err,resData){
                if (err) return res.status(500).send(err);
                apiresponse.data    = [];
                apiresponse.status  = true;
                apiresponse.message = "Banner has been Updated";
                res.json(apiresponse);
              });
            }
          } catch (err) {
            console.info("Bannerlist", err)
          }
        });
    });
  
  // insert banner
    router.post('/insertbanner', (req, res) => {
      var obj = { 
          "image"      : req.body.image,
          "button_txt"        : req.body.button_txt,
          "heading1"        : req.body.heading1,
      ...
      banner.create(obj,function(err,resdata){
        if(resdata){
          apiresponse.data    = [];
          apiresponse.status  = true;
          apiresponse.message = "Banner has been Inserted";
          res.json(apiresponse);
        }else{
          apiresponse.data    = [];
        apiresponse.status  = false;
        apiresponse.message = "Error on Inserted";
        res.json(apiresponse);
        }
    ...
    router.post('/bannerstatus',common.admintokenMiddleware, common.originMiddleware, (req, res) => {
      banner.findOneAndUpdate({ "_id": req.body._id },
        { "$set": 
          ...
          try {
            if(err) {
              res.status(500).send(err);
                ...
                apiresponse.status  = true;
                apiresponse.message = "Banner status has been Updated";
                res.json(apiresponse);
            }
          } catch (err) {
            console.info("Bannerlist", err)
